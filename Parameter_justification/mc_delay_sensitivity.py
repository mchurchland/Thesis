#!/usr/bin/env python3
"""
Delay-sensitivity validation for MC (memory capacity).

This mirrors ipc_delay_sensitivity.py, but evaluates MC instead of IPC:
1) capture of MC(D_max) versus a reference delay D_ref
...
def main() -> None:
                ...
                for input_scale in args.input_scale:
                    for model in models:
                        ...
                        for d in d_values:
                            ...
                            raw_rows.append((model, seed, rho, leak, input_scale, d, float(mc_total)))

    # Compute capture vs D_ref
    ref_map: dict[tuple, float] = {}
    for row in raw_rows:
        model, seed, rho, leak, in_scale, d, mc_total = row
        if int(d) == int(d_ref):
            ref_map[(model, seed, rho, leak, in_scale)] = float(mc_total)

    capture_rows: list[tuple] = []
    # (model, seed, rho, leak, input_scale, d_max, mc_total, capture_pct_vs_ref)
    for row in raw_rows:
        model, seed, rho, leak, in_scale, d, mc_total = row
        ref = ref_map.get((model, seed, rho, leak, in_scale), np.nan)
        cap = (100.0 * float(mc_total) / ref) if np.isfinite(ref) and ref > 0 else np.nan
        capture_rows.append((model, seed, rho, leak, in_scale, d, float(mc_total), cap))
    ...
    raw_csv = out_dir / "mc_delay_sensitivity_raw.csv"
    summary_csv = out_dir / "mc_delay_sensitivity_summary.csv"
    rank_csv = out_dir / "mc_delay_rank_stability.csv"
    report_txt = out_dir / "mc_delay_sensitivity_report.txt"

    _write_csv(
        raw_csv,
        ["model", "seed", "rho_target", "leak", "input_scale", "d_max", "mc_total", "capture_pct_vs_ref"],
        capture_rows,
    )
    _write_csv(
        summary_csv,
        ["model", "d_max", "mean_mc", "std_mc", "mean_capture_pct", "std_capture_pct", "n_runs"],
        summary_rows,
    )
    _write_csv(
        rank_csv,
    ...
    # Compact report
    lines: list[str] = []
    lines.append("MC delay sensitivity report")
    lines.append(f"D candidates={d_values}, D_ref={d_ref}")
    lines.append("")
